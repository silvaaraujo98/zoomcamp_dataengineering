...
Ingestão (Extract): Download dinâmico do CSV baseado nos inputs de year e month.

Staging Area: Os dados brutos são carregados em uma tabela intermediária (movimentacao_aeroportuaria_staging) com todos os campos como TEXT para evitar falhas de carga.

Limpeza e Transformação (Transform): * Criação de uma View de Limpeza (vw_movimentacao_aeroportuaria_limpa) que realiza o cast de tipos (Date, Time, Numeric).
...
Carga Final (Load): Utilização do comando MERGE (Upsert) para garantir a idempotência do pipeline, atualizando registros existentes e inserindo novos.
...
Instalação
Crie um novo Flow no seu painel do Kestra.

Copie e cole o conteúdo do arquivo pipeline_flight_anac.yaml.

Execute o workflow selecionando o mês e ano desejados nos inputs.
